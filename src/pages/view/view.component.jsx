...
const ViewPage = () => {
  ...
  return (
    <div className='view-page'>
      ...
      <div className='cards'>
        {
          arr.length != 0 ? (arr.map((item, i) => <Card itemId={item.id} itemName={item.name} itemCategory={capitalizeFirstLetter(item.category)} itemDescription={item.description} itemIngredients={item.ingredients.toString().replaceAll(',', ', ')} itemPrice={item.price} image={item.image} i={i} ctr={0} />))
